...
- 新: `/gbp-diag 事業者名 住所` → `node fetch.js "事業者名" "住所"`

移行方法: 旧形式のエリア（例: 「松戸」）だけが送られてきた場合は、そのまま実行せず
「住所（番地まで）を送ってください」と返信する。住所は対象特定（同名法人対策）と
町名スコープの両方に使われるため必須。

## 業種引数の廃止（2026-07-17・破壊的変更）

業種の手動指定を廃止した。競合検索・「主カテゴリの適切性」採点とも、Googleに
実際登録されているprimaryType/typesのみを情報源とする。理由: 診断者が選んだ業種と
実データが食い違うと、競合検索の基準と採点の基準が別々になり自己矛盾したレポートが
生成される事象が発生したため（gbp-scoring-rules.md参照）。
