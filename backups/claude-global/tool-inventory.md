...
| x-bookmarks（launchd loop） | loop | Xブックマーク閲覧相当(強権限cookie)＋Drive「X」フォルダ読み書き(サービスアカウント)＋Telegram直接送信 | 中（cookie失効で停止するのみ・データはSQLiteにも保持され復元可。ただしauth_tokenの漏洩はアカウント乗っ取りリスク） | 自作（X内部API直叩き。X側の仕様変更で壊れうる） | 不可（公式APIは有料のため今回不採用） | 維持 |
| x-grok（launchd loop） | loop | x-bookmarksと同じcookie共有＋同じDrive「X」フォルダ・サービスアカウント | 中（x-bookmarksと同一のリスク構造。cookie共有のため失効時は両方同時に止まる） | 自作（X内部API直叩き） | 不可 | 維持 |
